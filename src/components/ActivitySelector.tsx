...
export function ActivitySelector({ onSelect }: ActivitySelectorProps) {
  return <View style={styles.grid}>{activities.map((activity) => <Pressable accessibilityRole="button" accessibilityLabel={`Choose ${activity.label}`} onPress={() => onSelect?.(activity.id)} key={activity.label} style={({ pressed }) => [styles.card, pressed && styles.pressed]}><View style={[styles.icon, { backgroundColor: `${activity.color}18` }]}><Ionicons name={activity.icon} color={activity.color} size={22} /></View><Text numberOfLines={1} style={styles.label}>{activity.label}</Text><Text numberOfLines={1} style={styles.detail}>{activity.detail}</Text></Pressable>)}</View>;
}
