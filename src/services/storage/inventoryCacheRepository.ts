...
export const inventoryCacheRepository={
  async get(ownerUserId:string){const row=await (await database()).getFirstAsync<CacheRow>('SELECT payload,refreshed_at FROM inventory_cache WHERE owner_user_id=?',ownerUserId);return row?{items:JSON.parse(row.payload) as InventoryItem[],refreshedAt:row.refreshed_at}:undefined;},
