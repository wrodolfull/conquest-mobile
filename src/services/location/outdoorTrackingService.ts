...
import {
  ingestNativeLocations,
  waitForLocationIngestion,
} from "./locationIngestion";
...
async function permissionsGranted(): Promise<
  Exclude<TrackingStartResult, { ok: true }> | undefined
> {
...
function requestStartupFix(): void {
  ...
  const timeout = setTimeout(() => {
    active = false;
  }, STARTUP_FIX_TIMEOUT_MS);
  void Location.getCurrentPositionAsync({
    accuracy: Location.Accuracy.BestForNavigation,
  })
    .then(async (location) => {
      if (active) await ingestNativeLocations([location]);
    })
    .catch(() => undefined)
  ...
  try {
    if (
      !(await Location.hasStartedLocationUpdatesAsync(
        CONQUEST_OUTDOOR_LOCATION_TASK,
      ))
    )
    ...
    await activeActivityRepository.markNativeTrackingStarted(
      sessionId,
      Date.now(),
    );
    return true;
...
export async function startOutdoorTracking(
  ...
  const permissionFailure = await permissionsGranted();
  if (permissionFailure) return permissionFailure;
  const target = await targetRepository.load(ownerUserId);
  const session = await activeActivityRepository.create(
    type,
    ownerUserId,
    Date.now(),
    target ?? undefined,
  ...
  if (await startNativeTracking(session.id)) {
    requestStartupFix();
...
export async function resumeOutdoorTracking(
  sessionId: string,
): Promise<TrackingStartResult> {
  const session = await activeActivityRepository.get();
  if (!session || session.id !== sessionId || session.status !== "interrupted")
    return { ok: false, reason: "unresolved-session" };
  const permissionFailure = await permissionsGranted();
  if (permissionFailure) return permissionFailure;
  if (!(await startNativeTracking(session.id)))
    return { ok: false, reason: "unavailable" };
...
export async function stopOutdoorTracking() {
  if (
    ...
    await Location.stopLocationUpdatesAsync(CONQUEST_OUTDOOR_LOCATION_TASK);
