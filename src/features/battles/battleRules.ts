...
export interface BattleState { eligible:boolean; reason:BattleIneligibleReason|null; energyCost:number; influenceReward:number; myInfluence:number; leaderInfluence:number; currentEnergy:number; battleAvailableAt:string|null; protectedUntil:string|null }
...
export function battleUnavailableCopy(state:BattleState,now=Date.now()):{title:string;detail:string}|null {
 ...
 switch(state.reason){
  case'INSUFFICIENT_PRESENCE':return{title:'BUILD MORE INFLUENCE',detail:`${state.myInfluence} / ${Math.ceil(state.leaderInfluence/2)} required`};
