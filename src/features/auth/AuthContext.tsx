...
interface Value { session: AuthSession | null; user: AuthSession['user'] | null; profile: PlayerProfile | null; progress: PlayerProgress | null; loading: boolean; configured: boolean; refreshPlayer: () => Promise<void>; signInWithEmail: (e:string,p:string)=>Promise<AuthResult>; signUpWithEmail:(e:string,p:string)=>Promise<AuthResult>; signInWithGoogle:()=>Promise<AuthResult>; signOut:()=>Promise<AuthResult> }
const AuthContext = createContext<Value | null>(null);
export function AuthProvider({ children }: PropsWithChildren) {
  const [session, setSession] = useState<AuthSession | null>(null); const [profile, setProfile] = useState<PlayerProfile | null>(null); const [progress, setProgress] = useState<PlayerProgress | null>(null); const [loading, setLoading] = useState(true);
  const refreshPlayer = useCallback(async () => { if (!session?.user.id) { setProfile(null); setProgress(null); return; } try { const [nextProfile, nextProgress] = await Promise.all([profileRepository.get(session.user.id), progressRepository.get(session.user.id)]); setProfile(nextProfile); setProgress(nextProgress); } catch { setProfile(null); setProgress(null); } }, [session?.user.id]);
  ...
  const value = useMemo<Value>(() => ({ session, user: session?.user ?? null, profile, progress, loading, configured: isSupabaseConfigured, refreshPlayer, signInWithEmail: authRepository.signInWithEmail, signUpWithEmail: authRepository.signUpWithEmail, signInWithGoogle: () => authRepository.signInWithOAuthProvider('google'), signOut: authRepository.signOut }), [session, profile, progress, loading, refreshPlayer]);
