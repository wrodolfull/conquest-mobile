...
function OutdoorSession({ type }: { type: OutdoorActivityType }) {
  ...
  const [phase, setPhase] = useState<'ready' | 'denied' | 'error' | 'tracking'>('ready');
  ...
  const start = async () => {
    ...
    try {
      ...
      const watcher = await Location.watchPositionAsync(LOCATION_OPTIONS, (location) => {
      ...
      setStartedAt(startTime); setNow(startTime); setPhase('tracking');
    } catch {
  ...
  const finish = async () => {
    ...
    await activityRepository.save(completed);
    router.replace({ pathname: '/activity/results', params: { type, activityId: completed.id } });
  };

  if (phase !== 'tracking') return <ActivityFlowShell eyebrow="OUTDOOR ACTIVITY" title={activity.label}><View style={styles.ready}><Ionicons name="navigate-circle" size={64} color={activity.color} /><Text style={styles.readyTitle}>{phase === 'denied' ? 'Location access required' : phase === 'error' ? 'GPS could not start' : 'Ready to start?'}</Text><Text style={styles.readyCopy}>{phase === 'denied' ? 'Outdoor tracking requires foreground location access. Enable it in device settings, or go back.' : phase === 'error' ? 'We could not start location tracking. Check that location services are on, then try again or go back.' : 'GPS will record your real route while this screen remains active.'}</Text>{phase !== 'denied' && <><Text style={styles.gps}>{phase === 'error' ? 'Your activity has not started' : 'GPS ready · High accuracy'}</Text><Pressable disabled={starting} onPress={() => void start()} style={[styles.start, starting && styles.starting]}><Text style={styles.startText}>{starting ? 'STARTING GPS…' : phase === 'error' ? 'TRY AGAIN' : 'START ACTIVITY'}</Text></Pressable></>}</View></ActivityFlowShell>;
