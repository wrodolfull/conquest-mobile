...
export default function ActivityResultsScreen() {
  ...
  useEffect(() => { const load=()=>{if(params.activityId)void activityRepository.find(params.activityId).then(setResult)};load();return activityRepository.subscribe(load); }, [params.activityId]);
  ...
  return <ActivityFlowShell eyebrow="ACTIVITY COMPLETE" title={type[0]!.toUpperCase() + type.slice(1)} canGoBack={false}>
    ...
    <Text style={styles.syncState}>{result.syncStatus==='synced'?'SERVER CONFIRMED':'ESTIMATED · PENDING SYNC'}</Text><Text style={styles.heading}>ROUTE</Text><View style={styles.map}>{start ? <RouteMap route={result.route} /> : <Text style={styles.empty}>No accepted GPS points were recorded.</Text>}</View>
