...
export default function ActivityResultsScreen() {
  ...
  const [result, setResult] = useState<StoredOutdoorActivity>();
  useEffect(() => { const load=()=>{if(params.activityId&&user)void activityRepository.find(params.activityId,user.id).then(setResult)};load();return activityRepository.subscribe(load); }, [params.activityId,user]);
  ...
  const impacts=result.authoritativeTerritoryImpacts??result.traversals;
  return <ActivityFlowShell eyebrow="ACTIVITY COMPLETE" title={type[0]!.toUpperCase() + type.slice(1)} canGoBack={false}>
    <View style={styles.primary}><Text style={styles.label}>DISTANCE</Text><Text style={styles.value}>{((result.authoritativeDistanceMeters ?? result.distanceMeters) / 1000).toFixed(2)}<Text style={styles.unit}> km</Text></Text><Text style={styles.muted}>{formatDuration(result.durationSeconds)} duration</Text></View>
    <View style={styles.stats}><Stat label="XP" value={`+${result.authoritativeXpEarned ?? result.xpEarned}`} /><Stat label="ENERGY" value={`+${result.authoritativeEnergyEarned ?? result.energyEarned}`} /><Stat label="TERRITORIES" value={`${impacts.length}`} /><Stat label="INFLUENCE" value={`+${result.authoritativeInfluenceEarned ?? result.influenceEarned}`} /></View>
    <Text style={[styles.syncState,result.syncStatus==='failed'&&styles.failed]}>{result.syncStatus==='synced'?'SERVER CONFIRMED':result.syncStatus==='syncing'?'SYNCING WITH SERVER':result.syncStatus==='failed'?'SYNC FAILED':'PENDING SYNC · ESTIMATED'}</Text>{result.syncStatus==='pending'?<Text style={styles.explanation}>Saved safely on this device. Territory influence becomes official after server confirmation.</Text>:null}{result.syncStatus==='failed'?<><Text style={styles.explanation}>{result.lastSyncError??'Your activity is safe on this device.'}</Text><Pressable onPress={()=>user&&void activitySyncService.syncPending(user.id,true)}><Text style={styles.retry}>RETRY SYNC</Text></Pressable></>:null}<Text style={styles.heading}>PRIVATE ROUTE</Text><View style={styles.map}>{start ? <RouteMap route={result.route} /> : <Text style={styles.empty}>No accepted GPS points were recorded.</Text>}</View>
